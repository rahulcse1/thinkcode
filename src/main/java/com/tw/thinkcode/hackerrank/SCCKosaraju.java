...
public class SCCKosaraju {
  ...
  public static List<ArrayList<Integer>> kosaRaju(List<ArrayList<Integer>> graph) {
    ...
    for (int i = 0; i < n; i++) if (!visited[i]) dfs(graph, visited, order, i);
  ...
  static void dfs(
      List<ArrayList<Integer>> graph, boolean[] visited, List<Integer> order, int node) {
    visited[node] = true;
    for (int v : graph.get(node)) if (!visited[v]) dfs(graph, visited, order, v);
    order.add(node);
  }
